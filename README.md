...
- M-Team API 签名检测
- 上传量、下载量、分享率读取与定期通知
- 默认 25 天自动检测一次，并在同一条通知里发送上传量、下载量、分享率和网页登录提醒
...
11. 开启“25 天综合提醒”，综合提醒间隔填 `25`。
12. 保存后点击“检测”或“立即检测全部”。

说明：容器检测只能确认当前 Cookie / Authorization 还可用，不一定等同于站点规则里的“真实网页登录”。收到 25 天综合提醒后，微信通知里会同时带登录状态、上传量、下载量、分享率和站点首页链接，直接点击打开手动登录/刷新一次即可。
